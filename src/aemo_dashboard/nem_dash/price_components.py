...
def load_price_data():
    """
    Load price data using the dashboard's price file configuration
    """
    try:
        # Use the dashboard's configured spot_hist_file
        ...
        if 'REGIONID' in data.columns and 'RRP' in data.columns:
...
def create_price_table(prices):
    ...
    """
    if prices.empty:
        return pn.pane.HTML("<div>No price data available</div>", width=450, height=200)
    ...
    try:
        ...
        return pn.pane.DataFrame(styled_table, sizing_mode='fixed', width=450, height=280)
        
    except Exception as e:
        logger.error(f"Error creating price table: {e}")
        return pn.pane.HTML(f"<div>Error creating price table: {e}</div>", width=450, height=200)
...
def create_price_chart(prices):
    ...
    """
    if prices.empty:
        return pn.pane.HTML("<div>No price data available</div>", width=450, height=250)
    ...
    try:
        ...
        if rows_to_take == 0:
            return pn.pane.HTML("<div>No valid price data points</div>", width=450, height=250)
        ...
        return pn.pane.Matplotlib(fig, sizing_mode='fixed', width=450, height=250)
        
    except Exception as e:
        logger.error(f"Error creating price chart: {e}")
        return pn.pane.HTML(f"<div>Error creating price chart: {e}</div>", width=450, height=250)
